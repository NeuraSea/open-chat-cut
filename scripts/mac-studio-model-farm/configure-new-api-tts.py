#!/usr/bin/env python3
"""Idempotently expose the loopback Qwen3-TTS service through New API."""
...
SELECT
    ...
    'occ-tts', 1, 'OpenChatCut TTS', 0,
    extract(epoch FROM now())::bigint,
    'http://host.docker.internal:8192',
    'occ-tts', 'default,vip,svip',
    '{{"occ-tts":"Qwen3-TTS-12Hz-1.7B-CustomVoice"}}',
    0, 0, 'Private Mac Studio Qwen3-TTS service',
...
WHERE NOT EXISTS (SELECT 1 FROM channels WHERE name = 'OpenChatCut TTS');
...
SET type = 1,
    ...
    test_model = 'occ-tts',
    status = 1,
    base_url = 'http://host.docker.internal:8192',
    models = 'occ-tts',
    "group" = 'default,vip,svip',
    model_mapping = '{{"occ-tts":"Qwen3-TTS-12Hz-1.7B-CustomVoice"}}',
    auto_ban = 0,
    remark = 'Private Mac Studio Qwen3-TTS service'
WHERE name = 'OpenChatCut TTS';

DELETE FROM abilities
WHERE channel_id IN (SELECT id FROM channels WHERE name = 'OpenChatCut TTS')
  AND model <> 'occ-tts';

INSERT INTO abilities ("group", model, channel_id, enabled, priority, weight, tag)
SELECT desired_group, 'occ-tts', channel.id, true, channel.priority,
       channel.weight, channel.tag
FROM channels AS channel
CROSS JOIN (VALUES ('default'), ('vip'), ('svip')) AS groups(desired_group)
WHERE channel.name = 'OpenChatCut TTS'
...
"""
    ...
    if "COMMIT" not in result.stdout:
        raise RuntimeError("New API did not confirm the TTS transaction")
    print(f"Configured OpenChatCut TTS on {primary}")
...
def main() -> None:
    ...
    try:
        ...
        print(f"Failed to configure New API TTS: {error}", file=sys.stderr)
